...
namespace SummonersTable
{
    public sealed partial class TableBoard
    {
        ...
        void SyncSpells(MatchState state,double clock)
        {
            ...
            {active.Add(u.uid);if(!stunned.ContainsKey(u.uid))stunned[u.uid]=stun.Persistent(SlotPosition(p.seat,u.slot,count)+Vector3.up*.4f,transform);}
        ...
        {var prefab=cardLibrary.Find(id)?.spellEffect;if(prefab==null)return;var fx=Instantiate(prefab,transform);spellInstances.Add(fx);fx.Play(from,targets,ViewCamera);}
