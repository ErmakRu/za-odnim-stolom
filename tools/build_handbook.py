...
OUT=ROOT/'output/pdf/arena-test-decks-v0.2.pdf'
...
C.setTitle('Арена призыва - тестовые колоды 0.2.0')
W,H=A4
...
def begin(title,subtitle=''):
 ...
 C.setFont('Bold',9);C.setFillColor(MUTED);C.drawString(38,H-36,'АРЕНА ПРИЗЫВА / ТЕСТ 0.2.0')
...
y=H-400
y=block('2-4 игрока / 3 раунда / 30 уникальных карт','Каждый играет за себя. В каждой готовой колоде 30 экземпляров: 15 существ, 12 заклинаний и 3 реакции. Временная помощь соперникам разрешена.',y)
...
para('Версия баланса 0.2.0: исходный набор для тестов. Игра и PDF используют один каталог данных.',38,y,W-76,10,MUTED)

y=begin('Как проходит матч')
for title,txt in [
 ('Цель и очки','Если все оставшиеся герои погибают одновременно, им по 1 очку за ничью. Три раунда. Последний выживший получает 3 очка; устранение вражеского героя приносит 1 очко источнику урона. Между раундами здоровье, руки, поля и колоды обновляются; очки сохраняются. Первый игрок меняется каждый раунд. Побеждает максимальный общий счёт; при равенстве победа общая.'),
...
for title,txt in [
 ...
 ('Успешный призыв','Выберите карту, ячейку и цель. Карта видна всем в воздухе 3 секунды. Затем только владелец видит QTE. После успеха атака разрешается, выжившее существо выходит на стол с постоянным эффектом. При атаке существа действует ответный урон; погибший от ответа призываемый боец на стол не выходит.'),
 ...
 ('Реакции','По одной подходящей реакции от соперника во время показа карты или чужого QTE. Карта реакции сразу ложится на стол; защита, помощь, отражение и отмена срабатывают при успехе ритуала. При срыве реакция всё равно потрачена. После QTE и на обычные атаки со стола реакции закрыты.'),
 ('Постоянные эффекты','Работают пока существо на столе. Возврат в руку снимает эффект. Пределы складывания указаны на картах. Смерть героя убирает его существ. В локальной игре таймер QTE приостанавливается, пока другие выбирают реакции (до 8 секунд на ответ).')]:y=block(title,txt,y)
para('Стрелка в центр: случайная допустимая цель определяется после QTE. Для существ это всегда вражеский герой. Защитная реакция сработает, только если случайный удар попадёт в защищаемую цель.',38,y,W-76,10,MUTED)
