...
CONFIG=ROOT/'SummonersTable/Assets/StreamingAssets/Config'
DATA=CONFIG/'cards.json'
ART=ROOT/'SummonersTable/Assets/Resources/Art'
D={}
for config_name in ['cards.json','decks.json','rules.json']:
 D.update(json.loads((CONFIG/config_name).read_text(encoding='utf-8')))
VERSION=D['version']
OUT=ROOT/('output/pdf/arena-test-decks-v'+'.'.join(VERSION.split('.')[:2])+'.pdf')
...
C.setTitle('Арена призыва - тестовые колоды '+VERSION)
W,H=A4
...
def begin(title,subtitle=''):
 ...
 C.setFont('Bold',9);C.setFillColor(MUTED);C.drawString(38,H-36,'АРЕНА ПРИЗЫВА / ТЕСТ '+VERSION)
...
y=block('Главное правило','Существа и заклинания проходят QTE. Реакции играются без него. После QTE существо опускается на стол и ждёт назначения цели. Кнопка «Закончить ход» запускает атаки; после первой атаки включается постоянный эффект.',y)
...
para('Версия '+VERSION+': два режима и настраиваемое суммирование усилений. Игра и PDF используют данные Config.',38,y,W-76,10,MUTED)

y=begin('Выбор правил в лобби','Лидер выбирает настройки до старта. В течение матча они зафиксированы.')
y=block('Волшебники','До 3 заклинаний ИЛИ 1 заклинание и 1 существо за ход. Панель справа возле кнопки конца хода показывает оба варианта. После существа вариант с тремя заклинаниями тускнеет; после второго заклинания тускнеет смешанный вариант. Счётчики показывают использованные розыгрыши.',y)
y=block('Полководцы','Общий бюджет - 10 QTE за ход. Цена равна QTE, напечатанному на существе или заклинании. Можно смешивать типы и призывать несколько существ в свободные слоты. Пример: 4 + 4 + 2 = 10. В текущем наборе существо с QTE 2 отсутствует, но есть заклинание S07 с этой ценой.',y)
y=block('Когда тратится бюджет','Ресурс тратится при принятом розыгрыше, до QTE. Срыв, отмена эффекта или возврат в руку его не возвращают. Недопустимая цель или занятый слот ничего не тратят. Реакции бесплатны. Изменение длины ритуала существами и S07 не меняет напечатанную цену. Бюджет обновляется в начале хода.',y)
y=block('Флажок «Увеличение силы»','ВКЛ: действуют пределы суммирования из описаний. ВЫКЛ: постоянные усиления, защита и помехи суммируются без этих пределов, а ограничивающие фразы исчезают из карточек. Пять медведей дают защиту 2 с флажком и 5 без него. Максимальные HP 30, рука 8 карт, поле 5 слотов и минимальные 6 секунд QTE сохраняются.',y)
y=block('Как читать каталог ниже','Описания показаны для включённого флажка. Без него ограничения вида «не выше +2», «не более 4 HP» и «не складывается» для постоянных эффектов не действуют. Несколько суккубов прощают несколько промахов; повторный S07 складывает усиление и дополнительные символы.',y)
y=block('Флажок «3D карточки»','ВКЛ: иллюстрации выглядят утопленными внутрь рамки, смещаются с углом взгляда и слегка переливаются. ВЫКЛ: обычные плоские иллюстрации. Эффект виден у всех участников и не меняет правила. Рубашки возле героев скрывают содержимое руки при любом ракурсе.',y)
...
for title,txt in [
 ...
 ('Ваш ход','Нажмите своё существо и укажите стрелкой врага или центр стола. Стрелки сохраняются между ходами. Волшебники: 1 существо и до 1 заклинания либо до 3 заклинаний. После второго заклинания призыв недоступен. Полководцы: общий бюджет 10 QTE. Реакции не расходуют эти лимиты. На решения отведено 45 секунд, при розыгрыше этот таймер приостанавливается.'),
...
for title,txt in [
 ...
 ('Постоянные эффекты','Новый призыв включает эффект после первой атаки. Копия R02 - сразу, но атакует только с хода своего владельца. Возврат в руку снимает эффект. Пределы складывания действуют при включённом флажке «Увеличение силы». Смерть героя убирает его существ. В локальной игре таймер QTE приостанавливается, пока другие выбирают реакции (до 8 секунд на ответ).')]:y=block(title,txt,y)
...
print(json.dumps(dict(path=str(OUT),pages=len(reader.pages),cards=30,catalogueSha256=hashlib.sha256(b''.join((CONFIG/name).read_bytes() for name in ['cards.json','decks.json','rules.json'])).hexdigest()),ensure_ascii=False))
