...
OUT=ROOT/'output/pdf/arena-test-decks-v0.1.pdf'
...
C.setTitle('Арена призыва - тестовые колоды 0.1.0')
W,H=A4
...
def begin(title,subtitle=''):
 ...
 C.setFont('Bold',9);C.setFillColor(MUTED);C.drawString(38,H-36,'АРЕНА ПРИЗЫВА / ТЕСТ 0.1.0')
...
y=H-400
y=block('2-4 игрока / 3 раунда / 30 уникальных карт','Каждый играет за себя. В каждой готовой колоде 30 экземпляров: 12 существ, 12 заклинаний и 6 реакций. Временная помощь соперникам разрешена.',y)
y=block('Главное правило','Существа и заклинания проходят QTE. Реакции играются без него. Новое существо сначала атакует, затем выходит на стол и включает постоянный эффект.',y)
...
para('Версия баланса 0.1.0: исходный набор для тестов. Игра и PDF используют один каталог данных.',38,y,W-76,10,MUTED)

y=begin('Как проходит матч')
for title,txt in [
 ('Цель и очки','Три раунда. Последний выживший получает 3 очка; устранение вражеского героя приносит 1 очко источнику урона. Между раундами здоровье, руки, поля и колоды обновляются; очки сохраняются. Первый игрок меняется каждый раунд. Побеждает максимальный общий счёт; при равенстве победа общая.'),
 ...
 ('Ваш ход','Назначьте цели уже стоящим существам. Можно сыграть 1 существо и до 1 заклинания либо до 3 заклинаний без существа. После второго заклинания призыв недоступен. Реакции не расходуют эти лимиты. На решения отведено 45 секунд; QTE и реакции используют собственные таймеры.'),
 ('Боевая фаза','QTE существа завершает фазу действий после разрешения розыгрыша. Можно закончить ход кнопкой. Готовые существа атакуют слева направо. Новое существо второй раз в этот ход не бьёт. Без назначенной цели выбирается герой случайного живого противника. Если назначенное существо уже погибло, цель также заменяется случайным героем.'),
...
for title,txt in [
 ...
 ('Успешный призыв','Выберите карту, ячейку и цель. После QTE объявляется атака и открывается окно реакций. Затем атака разрешается, выжившее существо выходит на стол и включает постоянный эффект. При атаке существа действует ответный урон; погибший от ответа призываемый боец на стол не выходит.'),
 ('Срыв','До ритуала игра выбирает и показывает случайного живого противника - получателя при срыве. Провал отдаёт ему карту в руку, без атаки и эффекта. При полной руке карта сгорает. В дуэли получатель всегда второй игрок.'),
 ('Реакции','На объявленную атаку или враждебное заклинание даётся до 7 секунд. Каждый живой участник может сыграть одну подходящую реакцию или пропустить. Реакций на реакции нет. Защита и отмена применяются до урона; ответный урон - после. Выбор участников обрабатывается в порядке мест за столом.'),
 ('Постоянные эффекты','Работают пока существо живо и на столе. Возврат в руку снимает эффект. Пределы складывания указаны на картах. Пассивный ответ и эффекты начала хода не открывают реакции. Смерть героя убирает его существ.')]:y=block(title,txt,y)
para('Если последнее действие одновременно устраняет всех оставшихся героев, эти участники получают по 1 очку за ничейный раунд вместо 3 за победу.',38,y,W-76,10,MUTED)
...
for d in D['decks']:
 y=begin(d['name'],d['subtitle']+' / 30 карт: каждая позиция x2')
 ...
 for kind in ['creature','spell','reaction']:
  ...
  for e in d['entries']:
   ...
   C.setFont('Bold',10);C.setFillColor(MUTED);C.drawRightString(W-40,y+7,'x2')
  y-=14
...
for start in range(0,len(D['cards']),2):
 ...
 for c in D['cards'][start:start+2]:
  ...
  meta=f"{c['faction']} / {c['role']}" if c['kind']=='creature' else ('Разовый эффект' if c['kind']=='spell' else 'Ответ на событие')
  if c['kind']=='creature':
