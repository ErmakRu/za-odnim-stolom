...
OUT=ROOT/'output/pdf/arena-test-decks-v0.3.pdf'
...
C.setTitle('Арена призыва - тестовые колоды 0.3.0')
W,H=A4
...
def begin(title,subtitle=''):
 ...
 C.setFont('Bold',9);C.setFillColor(MUTED);C.drawString(38,H-36,'АРЕНА ПРИЗЫВА / ТЕСТ 0.3.0')
...
para('Версия баланса 0.3.0: исходный набор для тестов. Игра и PDF используют один каталог данных.',38,y,W-76,10,MUTED)
...
for title,txt in [
 ...
 ('Успешный призыв','Выберите существо и свободный слот, либо перетащите карту на слот. Карта показывается всем 2 секунды. Владелец вводит QTE; соперники видят только огоньки прогресса и оставшихся попыток. После успеха существо ждёт цели. Оно атакует в конце хода, получает ответный урон от существа и, если выживает, включает свой постоянный эффект.'),
...
para('Заклинание: цель до QTE, эффект сразу после успеха; центр выбирает случайную допустимую цель. Существо: цель после QTE, атака в конце хода; центр всегда выбирает вражеского героя. Камера: колесо меняет вид, зажатая ПКМ вращает взгляд.',38,y,W-76,10,MUTED)
