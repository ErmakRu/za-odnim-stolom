...
- Сетевой протокол 4, отдельный фильтр лобби 0.3.0. Всем игрокам нужен одинаковый билд.

## Запуск

Распакуйте **всю папку** из `output/ZaOdnimStolom-Windows-v0.4.0.zip`, запустите Steam, затем `ZaOdnimStolom.exe`. Для онлайна нужны разные Steam-аккаунты и ПК. Без Steam доступна игра за одним ПК. Версия баланса и PDF остаётся 0.3.0; 0.4.0 добавляет представление, внешний вид и его сетевую синхронизацию.

- Unity-проект: `SummonersTable`, редактор **6000.3.21f1**, Windows Build Support.
- Локальная сборка: `Builds/Windows-v0.4.0/ZaOdnimStolom.exe`.
